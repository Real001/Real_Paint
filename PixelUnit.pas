...
procedure TPixelForm.Button1Click(Sender: TObject);
begin
  img.Assign(buffer);
  MainForm.Log('Использован эффект: Укрупнение пикселов');
  PixelForm.Close;
end;

procedure TPixelForm.Button2Click(Sender: TObject);
begin
  buffer.Assign(img);
...
procedure TPixelForm.EditChange(Sender: TObject);
begin
  buffer.Assign(img);
...
procedure TPixelForm.TrackBarChange(Sender: TObject);
begin
